...
{
    {8,     0xFFE0, ATTR_SERVICE,        0, 0,    0},
    {0,     0xFFE2, ATTR_CHARACTISTIRC,  0, 0x08, 0x11},
    {0,     0xFFE1, ATTR_CHARACTISTIRC,  0, 0x18, 0x11},
...
static void ble_server_included_service_added_cb(int status, int server_if, int srvc_handle, int incl_srvc_handle)
{
    TLS_BT_APPL_TRACE_API("%s , status=%d\r\n", __FUNCTION__, status);
}
...
static void ble_server_descriptor_added_cb(int status, int server_if, uint16_t descr_id, int srvc_handle, int descr_handle)
{
    TLS_BT_APPL_TRACE_API("%s , status=%d\r\n", __FUNCTION__, status);
...
static void ble_server_request_read_cb(int conn_id, int trans_id, tls_bt_addr_t *bda, int attr_handle, int offset, bool is_long)
{
...
static void ble_server_request_write_cb(int conn_id, int trans_id, tls_bt_addr_t *bda, int attr_handle, int offset, int length, bool need_rsp, bool is_prep, uint8_t *value)
{	
    ...
    else
    {
        if(g_ble_output_fptr) 
            g_ble_output_fptr(value, length);
    }
}
...
static void ble_server_request_exec_write_cb(int conn_id, int trans_id, tls_bt_addr_t *bda, int exec_write)
{
}
