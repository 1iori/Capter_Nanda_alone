...
void app_adapter_properties_callback(tls_bt_status_t status, int num_properties, tls_bt_property_t *properties)
{
	tls_bt_host_msg_t msg;
...
void app_remote_device_properties_callback(tls_bt_status_t status, tls_bt_addr_t *bd_addr, int num_properties, tls_bt_property_t *properties)
{
    int i = 0;
...
void app_bond_state_changed_callback(tls_bt_status_t status, tls_bt_addr_t *remote_bd_addr, tls_bt_bond_state_t state)
{
...
void app_acl_state_changed_callback(tls_bt_status_t status, tls_bt_addr_t *remote_bd_addr, tls_bt_acl_state_t state, uint8_t link_type)
{
...
void app_ssp_request_callback(tls_bt_addr_t *remote_bd_addr, tls_bt_bdname_t *bd_name, uint32_t cod, tls_bt_ssp_variant_t pairing_variant, uint32_t pass_key)
{
...
void app_pin_request_callback(tls_bt_addr_t *remote_bd_addr, tls_bt_bdname_t *bd_name, uint32_t cod, uint8_t min_16_digit)
{
...
static void tls_bt_host_callback_handler(tls_bt_host_evt_t evt, tls_bt_host_msg_t *msg)
{
	...
	switch(evt)
	{
		...
		case WM_BT_RMT_DEVICE_PROP_EVT:
			app_remote_device_properties_callback(msg->remote_device_prop.status, msg->remote_device_prop.address, msg->remote_device_prop.num_properties, msg->remote_device_prop.properties);
			break;
...
{
    if((event != WM_BLE_DM_SCAN_RES_EVT) && (event != WM_BLE_DM_SCAN_RES_CMPL_EVT)) 
        return;
...
int tls_ble_demo_scan(uint8_t start)
{
    ...
    if(start)
    {
        tls_ble_set_scan_param(0x40, 0x60, 1);
...
*   ble服务端接收到数据回调
*/
static void user_ble_server_event_handle(uint8_t *p_data, uint32_t length)
{
    printf("\r\nrec data:%s\r\n", p_data);
}
...
{
    tls_ble_server_demo_api_send_msg(data, length);
}
...
{
    if(callback != NULL)
        tls_ble_server_demo_api_init(callback);
    else
        tls_ble_server_demo_api_init(user_ble_server_event_handle);
}
...
*   ble扫描回调
*/
static ble_scan_event_callback_t ble_scan_bk = NULL;
static void user_ble_scan_event_handle(tls_ble_dm_evt_t event, tls_ble_dm_msg_t *p_data)
{
    if(event == WM_BLE_DM_SCAN_RES_EVT)
    {
        tls_ble_dm_scan_res_msg_t *msg = (tls_ble_dm_scan_res_msg_t *)&p_data->dm_scan_result;

        if(msg->value[5] == 0x48 && msg->value[6] == 0x45)
        {
            if(ble_scan_bk != NULL)
            {
                ble_scan_bk(msg->value);
            }
        }
    }
}
...
void user_ble_scan_start(ble_scan_event_callback_t callback)
{
    ...
    {
        bt_adapter_scaning = 1;
        ret = tls_ble_register_report_evt(WM_BLE_DM_SCAN_RES_EVT|WM_BLE_DM_SCAN_RES_CMPL_EVT, user_ble_scan_event_handle);
    }
}
