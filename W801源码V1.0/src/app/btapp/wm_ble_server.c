...
void btgatts_descriptor_added_cb(int status, int server_if, tls_bt_uuid_t *descr_id, int srvc_handle, int descr_handle)
{
    int index = -1;
    uint16_t app_uuid;
    index = get_app_env_index_by_server_if(server_if);
...
void btgatts_request_read_cb(int conn_id, int trans_id, tls_bt_addr_t *bda, int attr_handle, int offset, bool is_long)
{
    int index = -1;
...
void btgatts_request_write_cb(int conn_id, int trans_id, tls_bt_addr_t *bda, int attr_handle, int offset, int length, bool need_rsp, bool is_prep, uint8_t *value)
{
    int index = -1;
//	TLS_BT_APPL_TRACE_VERBOSE("%s,conn_id=%d, trans_id=%d, attr_handle=%d\r\n", __FUNCTION__, conn_id, trans_id, attr_handle);
    if(need_rsp)
...
void btgatts_request_exec_write_cb(int conn_id, int trans_id, tls_bt_addr_t *bda, int exec_write)
{
    int index = -1;
    index = get_app_env_index_by_conn_id(conn_id);
    if(index<0)
